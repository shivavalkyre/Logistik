...
<div style="margin:20px 0 10px 0;">
        <div style="margin-bottom:20px">
            <input id="tgl_penerimaan" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" prompt="Tanggal Penerimaan" style="width:18%;">
            <input id="no_penerimaan" class="easyui-textbox" prompt="Nomor Penerimaan" style="width:18%;">
	...
	$(document).ready( function () {
		$('#hapus_btn').hide();
		  ...
		  //alert(transaksi_id);
			...
			$.ajax({
			type	: 'POST',
			url		: "../control/get_transaksi_receive_selected.php",
			data	: string,
			cache	: false,
			success	: function(data){
				...
				$('#tgl_penerimaan').datebox('setValue',obj["rows"][0].tanggal);
				$('#no_penerimaan').textbox('setValue',obj["rows"][0].no_terima);
				$('#no_bast').textbox('setValue',obj["rows"][0].no_bast);
				//console.log();
				$('#hapus_btn').show();
				$('#dg1').datagrid({
					url:'../control/get_transaksi_receive_selected_detail.php?transaksi_id='+transaksi_id,
				});
			}
		});
			}
		...
		$('#btnSave').bind('click', function(){
			...
			var tgl_penerimaan = $('#tgl_penerimaan').datebox('getValue');
			var no_penerimaan = $('#no_penerimaan').textbox('getValue');
			var no_bast = $('#no_bast').textbox('getValue');
			var user_id = '<?php echo"$user_id"?>';
			var transaksi_id = '<?php echo"$transaksi_id"?>';
			//alert(user_id);

			if(no_bast!=='' && no_penerimaan!=='' && no_bast!==''){
			if (total>0){
			if (title==='tambah'){
								//ajax save transaction
			var string = "tgl_penerimaan="+tgl_penerimaan+"&no_penerimaan="+no_penerimaan+"&no_bast="+no_bast+"&user_id="+user_id;
			//alert (string);	
			$.ajax({
			type	: 'POST',
			url		: "../control/save_header_receive.php",
			data	: string,
			cache	: false,
			success	: function(data){
			//alert(data);
			...
			for (i=0;i<total;i++){
				...
				//alert (stringdetail);
									$.ajax({
									type	: 'POST',
									url		: "../control/save_detail_receive.php",
									data	: stringdetail,
									cache	: false,
									success	: function(data){
										//alert (data);
										...
										setTimeout(function(){
											...
											$('#tgl_penerimaan').textbox('setValue','');
											$('#no_penerimaan').textbox('setValue','');
			...
			//alert(title);
			var string = "tgl_penerimaan="+tgl_penerimaan+"&no_penerimaan="+no_penerimaan+"&no_bast="+no_bast+"&user_id="+user_id+"&transaksi_id="+transaksi_id;
			//alert (string);	
			$.ajax({
			type	: 'POST',
			url		: "../control/update_header_receive.php",
			data	: string,
			cache	: false,
			success	: function(data){
			//alert(data);
			...
			for (i=0;i<total;i++){
				...
				//alert (stringdetail);
									$.ajax({
									type	: 'POST',
									url		: "../control/update_detail_receive.php",
									data	: stringdetail,
									cache	: false,
									success	: function(data){
										//alert (data);
		...
		$('#hapus_btn').bind('click', function(){
			...
			$.ajax({
			type	: 'POST',
			url		: "../control/delete_detail_selected_receive.php",
			data	: string,
			cache	: false,
			success	: function(data){
				//alert(data);
